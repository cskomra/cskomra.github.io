...
function updatePositions() {
  ...
  //Efficient to access DOM -> document.getElementsByClass('.mover')
  ...
  for (var i = 0; i < items.length; i++) {
    /*What are the exact numbers that phase and document.body.scrollTop give me per iteration?
      ...
      Therefore we are calculating the same set of 5 numbers for all our pizzas no matter how big our list of pizzas are!*/
    var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));
    //what is value of (i % 5)? just only ever 5 #s.  store those 5 and use them
    //Let's log out all these numbers and see!
    //console.log(phase, document.body.scrollTop / 1250)
    //which vars should leave inside for loop and which can take outside

    /*Using style.left is there a more efficient way to change the postion of this object?
     ...
     Perhaps CSS3 hardware acceleration can reduce the need trigger a re-layout? Can we offload the CPU and use the GPU (video cart) to help us
     The CSS 'transform' property can help us here.  */

    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';  //THIS IS FINE
  }
    //instead of style.left
    /* Advanced Hack Here: Can we also reduce the need for the browser to paint the entire scree?  Can we tell the browser to only repaint the ne
    actually moving?  Whenever a pixel in a layer changes, the browser repaints the entire layer. Therefore we can force the browser to render
    animating pizza in its own layer? Therefore whenever we animate the pizzas, only a small part of the screen
    //reduce the need to trigger a re-layout
    We should look up these CSS (hardware accelleration) Hacks to see if they can force our elements into its layer:
    ...
    Moving all of these pizzas in its own composite layer offloads the textureing and painting to the GPU
